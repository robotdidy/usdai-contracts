...
contract BaseTokenUpgradeTest is Test {
    ...
    /* Converter admin address */
    address internal converterAdmin;
    ...
    function setUp() public {
        ...
        converterAdmin = createUser("converterAdmin");
        deal(PYUSD, converterAdmin, 1_000_000_000 * 1e6);

        vm.prank(converterAdmin);
        IERC20(PYUSD).approve(USDAI, 1_000_000_000 * 1e6);
    ...
    function upgradeUSDai() internal {
        ...
        // Grant convert base token admin role to admin
        vm.prank(ADMIN);
        AccessControl(USDAI).grantRole(keccak256("CONVERT_BASE_TOKEN_ADMIN_ROLE"), converterAdmin);
    }
    ...
    function upgradeStakedUSDai() internal {
        ...
        ProxyAdmin(proxyAdminAddress).upgradeAndCall(
            ITransparentUpgradeableProxy(STAKED_USDAI), address(stakedUsdaiImpl), ""
        );
    }
    ...
    function test__UpgradeBaseToken() public {
        ...
        uint256 stakedUsdaiTotalSupplyBefore = IERC20(STAKED_USDAI).totalSupply();
        uint256 stakedUsdaiTotalBridgedSupplyBefore = stakedUsdai.bridgedSupply();
        uint256 stakedUsdaiTotalSharesBefore = stakedUsdai.totalShares();

        // DEV: should call depositBaseYield() before upgrade

        // Upgrade
        upgradeUSDai();
        upgradeStakedUSDai();
        // DEV: should upgrade queued depositor

        // Get after state
        uint256 usdaiTotalSupplyAfter = usdai.totalSupply();
        uint256 usdaiTotalBridgedSupplyAfter = usdai.bridgedSupply();

        uint256 navAfter = stakedUsdai.nav();
        uint256 redemptionSharePriceAfter = stakedUsdai.redemptionSharePrice();
        uint256 depositSharePriceAfter = stakedUsdai.depositSharePrice();
        uint256 stakedUsdaiTotalSupplyAfter = IERC20(STAKED_USDAI).totalSupply();
        uint256 stakedUsdaiTotalBridgedSupplyAfter = stakedUsdai.bridgedSupply();
        uint256 stakedUsdaiTotalSharesAfter = stakedUsdai.totalShares();

        // Assertions
        assertEq(usdaiTotalSupplyAfter, usdaiTotalSupplyBefore, "Total supply should be the same");
        assertEq(usdaiTotalBridgedSupplyAfter, usdaiTotalBridgedSupplyBefore, "Total bridged supply should be the same");

        assertApproxEqRel(navAfter, navBefore, 11e10, "Nav should be the same"); // ~0.000011%
        assertApproxEqRel(
            redemptionSharePriceAfter, redemptionSharePriceBefore, 11e10, "Redemption share price should be the same"
        ); // ~0.000011%
        assertApproxEqRel(
            depositSharePriceAfter, depositSharePriceBefore, 11e10, "Deposit share price should be the same"
        ); // ~0.000011%
        assertEq(stakedUsdaiTotalSupplyAfter, stakedUsdaiTotalSupplyBefore, "Total supply should be the same");
        assertEq(
            stakedUsdaiTotalBridgedSupplyAfter,
            stakedUsdaiTotalBridgedSupplyBefore,
            "Total bridged supply should be the same"
        );
        assertEq(stakedUsdaiTotalSharesAfter, stakedUsdaiTotalSharesBefore, "Total shares should be the same");

        // Get token balance before conversion
        uint256 wrappedMTokenBalance = IERC20(WRAPPED_M_TOKEN).balanceOf(USDAI);

        // Convert Wrapped M to PYUSD
        vm.prank(converterAdmin);
        USDai(USDAI).convertBaseToken(wrappedMTokenBalance);

        // Verify token balance after conversion
        uint256 wrappedMTokenBalanceAfter = IERC20(WRAPPED_M_TOKEN).balanceOf(USDAI);
        assertEq(wrappedMTokenBalanceAfter, 0, "Wrapped M token balance equal 0");
        assertEq(
            IERC20(PYUSD).balanceOf(USDAI), wrappedMTokenBalance, "PYUSD balance equal initial wrapped M token balance"
        );
        assertGe(
            IERC20(PYUSD).balanceOf(USDAI) * 1e12,
            usdai.totalSupply() + usdai.bridgedSupply(),
            "USDai is backed by PYUSD"
        );

        // Verify staked USDai
        assertEq(stakedUsdai.nav(), navAfter, "Nav should be the same");
        assertEq(
            stakedUsdai.redemptionSharePrice(), redemptionSharePriceAfter, "Redemption share price should be the same"
        );
        assertEq(stakedUsdai.depositSharePrice(), depositSharePriceAfter, "Deposit share price should be the same");
    }
